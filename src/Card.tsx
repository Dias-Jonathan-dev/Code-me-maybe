...
const Card: React.FC<CardProps> = ({
  ...
  const renderCardContent = () => {
    // Si la carte est de type 'name' ET qu'elle a un style appliqué
    // (ce qui se produit si level.showVisualOnNameCard est true),
    // nous rendons les éléments visuels réels ici.
    // Sinon, nous affichons juste le texte 'name'.
    if (display.type === "name" && Object.keys(display.style).length > 0) {
      // Pour les niveaux où les noms affichent aussi le visuel
      return (
        <div className="card-visual-content" style={display.style}>
          {/* Le contenu du nom est aussi affiché ici */}
          <div className="card-name-overlay">{display.content || ""}</div>
          {/* Les mêmes éléments de rendu visuel que pour une carte de type 'visual' */}
          {cardId === "font-size-2em" && (
            <span style={{ fontSize: "2em" }}>Texte Grand</span>
          )}
          {cardId === "font-weight-bold" && (
            <span style={{ fontWeight: "bold" }}>Texte Gras</span>
          )}
          {cardId === "font-style-italic" && (
          ...
          {cardId === "border-2px-dashed" && (
            <div className="visual-box" style={{ border: "2px dashed red" }}>
              Bordure
            </div>
          )}
          {cardId === "background-color-yellow" && (
            <div
              className="visual-full-box"
              style={{ backgroundColor: "yellow" }}
            ></div>
          )}
          {cardId === "box-shadow-medium" && (
            <div className="visual-box visual-box-shadow"></div>
          )}
          {cardId === "border-radius-50p" && (
            <div
          ...
          {cardId === "text-decoration-underline" && (
            <span style={{ textDecoration: "underline" }}>Texte Souligné</span>
          )}
          {cardId === "text-transform-uppercase" && (
            <span style={{ textTransform: "uppercase" }}>MAJUSCULES</span>
          )}
          {cardId === "position-relative" && (
            <div className="visual-positioned-box visual-relative">
              Relative
            </div>
          )}
          {cardId === "position-absolute" && (
            <div className="visual-positioned-box visual-absolute">Absolue</div>
          )}
          {cardId === "z-index-high" && (
            <div className="visual-positioned-box visual-z-index-high">
              Z-Index
            </div>
          )}
          {cardId === "overflow-hidden" && (
          ...
          {cardId === "text-overflow-ellipsis" && (
            <div className="visual-overflow-ellipsis-box">
              Ceci est un texte très long qui devrait être tronqué avec des
      ...
      // Pour une carte "nom" sans style visuel (niveaux précédents)
      return (
      ...
      // Pour une carte "visuel" (toujours afficher le visuel)
      return (
        <div className="card-visual-content" style={display.style}>
          {" "}
          {/* Style appliqué directement ici */}
  ...
  return (
    <div className={cardClassName} onClick={onClick}>
      <div className="card-inner">
        ...
        <div
          className="card-back"
          style={display.type === "visual" ? display.style : {}}
        >
          {/* Appliquer le style de display.style à la face arrière si c'est une carte visuelle,
              OU si c'est une carte de nom avec un style visuel à afficher. */}
          {renderCardContent()}
        </div>
      </div>
    </div>
